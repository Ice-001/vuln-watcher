...
func main() {
	...
	app.Flags = []cli.Flag{
		...
		&cli.StringFlag{
			Name:     "md-dir",
			Aliases:  []string{"md"},
			Usage:    "directory to save daily md reports",
			Category: "[Launch Options]",
		},
		&cli.BoolFlag{
		...
		&cli.BoolFlag{
			Name:     "once",
			Usage:    "run once and exit, no ticking",
			Category: "[Launch Options]",
		},
		&cli.BoolFlag{
			Name:     "insecure",
		...
		&cli.BoolFlag{
			Name:     "no-push",
			Aliases:  []string{"np"},
			Usage:    "disable message push, only save to md and db",
			Category: "[Launch Options]",
		},
		&cli.BoolFlag{
			Name:     "debug",
...
func initConfigFromCli(c *cli.Context) (*ctrl.WatchVulnAppConfig, error) {
	...
	if os.Getenv("NO_START_MESSAGE") == "true" {
		noStartMessage = true
	}
	if os.Getenv("NO_GITHUB_SEARCH") == "true" {
		noGithubSearch = true
	}
	...
	log.Infof("DIFF env value: '%s'", os.Getenv("DIFF"))
	if os.Getenv("DIFF") == "true" {
		diff = true
	}
	log.Infof("NO_PUSH env value: '%s'", os.Getenv("NO_PUSH"))
	if os.Getenv("NO_PUSH") == "true" {
		noPush = true
	}
